...
@section('page-subtitle', 'Organize an event for ' . $organization->name)
    ...
    <div class="col-md-10">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-gradient text-white" style="background: linear-gradient(135deg, #2d6a4f 0%, #52b788 100%);">
                <h5 class="mb-0"><i class="bi bi-calendar-plus me-2"></i>Create Event for {{ $organization->name }}</h5>
            </div>
            <div class="card-body p-4">
                <form method="POST" action="{{ route('organizer.events.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" name="type" id="event_type" required>
                                ...
                                <option value="onsite" {{ old('type') == 'onsite' ? 'selected' : '' }}>On-site</option>
                                <option value="hybrid" {{ old('type') == 'hybrid' ? 'selected' : '' }}>Hybrid (Online + On-site)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold">Upload Event Poster</label>
                        <input type="file" 
                               class="form-control @error('poster_path') is-invalid @enderror" 
                               name="poster_path" 
                               accept="image/*">
                        <small class="text-muted">Upload an eye-catching poster for your event (JPG, PNG, max 2MB)</small>
                        @error('poster_path')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        <strong>Note:</strong> The event will be saved as a draft. You can publish it later from the event management page.
                    </div>
